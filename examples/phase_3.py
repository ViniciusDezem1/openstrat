# -*- coding: utf-8 -*-
"""
Created on Tue Jan 30 14:42:24 2024
...
def analyze_strategy(strategy_name, task_names, s, t, c, w_time, w_cost):
    ...
    # Assigning weights and combining scores
    ...
    # Calculate the actual minimum and maximum scores
...
# Normalize score
all_scores = [score for _, score in strategy_1_combined_scores + strategy_2_combined_scores]
min_score = min(all_scores)
max_score = max(all_scores)
# Normalize the scores for both strategies
strategy_1_combined_scores = [(task, (score - min_score) / (max_score - min_score)) for task, score in strategy_1_combined_scores]
strategy_2_combined_scores = [(task, (score - min_score) / (max_score - min_score)) for task, score in strategy_2_combined_scores]
...
# Strategy 1
axes[0].barh(sorted_task_names_1, sorted_scores_1, color='#6495ED')
...
axes[0].set_yticks(range(len(sorted_task_names_1)))
axes[0].tick_params(axis='y', labelsize=9.5)  # Set font size separately
axes[0].set_yticklabels(sorted_task_names_1)
...
# Strategy 2
axes[1].barh(sorted_task_names_2, sorted_scores_2, color='#FF4500')
...
axes[1].invert_yaxis()
axes[1].set_yticks(range(len(sorted_task_names_2)))
axes[1].tick_params(axis='y', labelsize=9.5)  # Set font size separately
axes[1].set_yticklabels(sorted_task_names_2)
...
file_path = __file__.replace('.py', '_final_plot.png')  # Create a file path based on the Python script name
